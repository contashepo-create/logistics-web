...
## v25 — تعدّد المستخدمين الإضافيين لكل شركة

قبل هذه الترحيلة كان النظام يسمح بمستخدم إضافي **واحد** لكل شركة، بسبب فهرس
فريد جزئي أضافته v11 (`uq_profiles_one_additional_per_company`). هذا القيد
مصطنع ولا علاقة له بعزل البيانات: كل سياسات RLS تعتمد على
`public.auth_company_id()` التي تقرأ `company_id` من `profiles`، فأي عدد من
الصفوف بنفس الشركة يعمل بلا أي تعديل على السياسات.

📄 **`supabase/migration_multi_company_users_v25.sql`**

ما تفعله:

1. تضيف `companies.max_additional_users` (بين 0 و10، **الافتراضي 1**).
2. تحذف الفهرس الفريد للمستخدم الإضافي، وتُبقي «مالك واحد لكل شركة».
3. تفرض الحد بمشغّل `trg_profiles_additional_limit` مع قفل استشاري يمنع
   تجاوز الحد عند وصول طلبَي إنشاء متزامنين.
4. تضيف `admin_set_company_user_limit_v25(uuid, int)` لضبط الحد من لوحة
   المطوّر، وترفض خفضه دون عدد الحسابات القائمة.
5. تُحدّث `admin_get_company_extras_v18` لتعيد الحد وعدد المستخدمين.

### ⚠️ الملف لا يُغلَّف بمعاملة — لا تضف `begin/commit`

النسخة الأولى كانت داخل معاملة واحدة، فاحتفظت بـ `AccessExclusiveLock` على
`companies` ثم `profiles` طوال التنفيذ، بينما يقرأ التطبيق الحيّ (PostgREST)
الجدولين بالترتيب المعاكس، فينتج:

```text
ERROR:  40P01: deadlock detected
```

الملف الحالي ينفّذ كل عبارة DDL في معاملتها الضمنية القصيرة، مع
`lock_timeout = 4s` وإعادة محاولة تلقائية، ويضيف قيد الفحص بـ `NOT VALID` ثم
`VALIDATE` حتى لا يقفل الجدول أثناء الفحص. **لا تلفّه بمعاملة يدوياً.**

إذا حدث الجمود سابقاً فالترحيلة تراجعت بالكامل ولم يُطبَّق منها شيء — قاعدتك
سليمة، ويكفي إعادة تشغيل الملف الجديد.

### خطوات التشغيل

1. خذ نسخة احتياطية.
2. يُفضّل التنفيذ في وقت هدوء، أو أغلق تبويبات التطبيق المفتوحة لتقليل
   المعاملات المتزامنة.
3. نفّذ `supabase/migration_multi_company_users_v25.sql` في SQL Editor
   **كما هو ودفعة واحدة**.
4. من **لوحة المطوّر → مميزات إضافية** اختر الشركة واضبط «عدد المستخدمين
   الإضافيين المسموح بهم»، ثم أضف الحسابات.

إن ظهر إشعار «تعذّر الحصول على القفل» فهناك معاملة طويلة جارية:
...
انتظر انتهاءها ثم أعد تشغيل الملف — فهو آمن للتكرار بالكامل.

⚠️ الترحيلة لا تغيّر سلوك أي شركة قائمة: الجميع يبقى على حد = 1 حتى ترفعه
يدوياً. ولا تمسّ أي جدول تشغيلي (رحلات/فواتير/سندات) ولا تحذف أي بيانات.

### ملاحظات مهمة

- **الصلاحيات:** كل المستخدمين الإضافيين يرون بيانات الشركة كاملة بصلاحيات
  المالك نفسها. الصلاحيات المفصّلة (محاسب يرى الفواتير فقط… إلخ) غير مشمولة
  وتحتاج طبقة صلاحيات مستقلة عبر كل السياسات.
- **ميزة `additional_user`** صارت بوابة على مستوى الشركة تُزامَن تلقائياً:
  مفعّلة ما دام هناك مستخدم إضافي **نشط** واحد على الأقل. إيقاف حساب بعينه
  يتم عبر `profiles.is_active` ولا يقطع الوصول عن بقية الحسابات.
- **تفرّد الهاتف والبريد** بين الحسابات ما زال مفروضاً من v24، فلكل مستخدم
  إضافي بريد ورقم مختلفان.

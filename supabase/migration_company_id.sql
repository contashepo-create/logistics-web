...
begin
  for r in select * from public.profiles loop
    if r.company_name is null or btrim(r.company_name) = '' then
      continue;
    end if;
    insert into public.companies (name, phone, email)
    values (r.company_name, coalesce(r.phone, ''), r.email)
    returning id into v_company;
    update public.profiles set company_id = v_company where id = r.id;
    execute format(
      'update public.%I set company_id = %L where user_id = %L', 'financial_years', v_company, r.id);
  end loop;
end $$;

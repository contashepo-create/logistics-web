...
export async function pnlReport(
  ...
  if (invIds.length) {
    ...
    const tripIds = (trips ?? []).map((t) => t.id);
    if (tripIds.length) {
      const { data: exps } = await supabase.from("trip_expenses").select("amount, source").in("trip_id", tripIds);
      for (const e of exps ?? []) {
        // المصروف الذي يتحمّله العميل يُضاف للإيراد ولا يُعد تكلفة
        if ((e as { source?: string }).source === "customer") transport += num(e.amount);
        else direct += num(e.amount);
      }
    }
  }

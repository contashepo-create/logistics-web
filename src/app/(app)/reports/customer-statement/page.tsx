    ...
    <PageFrame title="كشف حساب عميل" subtitle="الرصيد الافتتاحي + الفواتير − سندات القبض = الرصيد الحالي"
      ...
      exportBar={<ExportBar
        onExcel={() => exportPage({ title: "كشف حساب عميل", subtitle, headers, rows, summaryLines: [
          ...
          ["إجمالي الفواتير (مدين)", money(invTotal)], ["إجمالي التحصيل (دائن)", money(recTotal)],
          ["إشعارات مدين", money(notesDebit)], ["إشعارات دائن", money(notesCredit)],
          ...
          <div className="table-wrap">
            <table className="data-table">
              ...
              <tbody>
                ...
                <tr style={{ fontWeight: 700 }}><td></td><td>الإجمالي / الرصيد النهائي</td><td></td><td>{money(invTotal + notesDebit)}</td><td>{money(recTotal + notesCredit)}</td><td>{money(st?.closing ?? 0)}</td></tr>
              </tbody>
            </table>
          </div>
